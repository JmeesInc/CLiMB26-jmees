#!/usr/bin/env python3
"""CLiMB submission v002 — DA3 submap SLAM + Kannala-Brandt rectification.
...
# expB06: pose-conditioned second pass. DA3 accepts extrinsics+intrinsics and
# injects them as camera tokens (model/cam_enc.py -> backbone slot 0). The
# submission never used it. "twopass": run the window unconditioned, chain it as
# usual, then re-run it conditioned on the chained global poses + the KNOWN
# rectified pinhole K, and keep the re-predicted poses of the new views
# (align_to_input_ext_scale=False returns Umeyama-aligned predictions).
COND = os.environ.get("DA3_COND", "off").lower()
# expB06 B3: window phase offset (in keyframes). Two chains with different
# phases have different window boundaries, so their per-window scale errors are
# largely independent; averaging the two trajectories (scripts/merge_phases.py)
# halves that variance if so. The first window always starts at 0 so coverage
# is complete.
PHASE = int(os.environ.get("DA3_PHASE", 0))
# expB11 anchor windows: prepend up to DA3_ANCHORS already-placed past keyframes
# at exponential offsets (step*2, step*4, ...) to every window. DA3 estimates
# them jointly with the current frames, and since their global poses are known
# they join the alignment set -- long-baseline scale constraints reach each
# window through co-visible near anchors (the missing piece of §14.3).
ANCHORS = int(os.environ.get("DA3_ANCHORS", 0))
ANCHOR_BASE = int(os.environ.get("DA3_ANCHOR_BASE", 2))
...
def build_rectify_maps(calib, w, h, balance=0.0):
...
class VideoWindow:
    ...
    def window(self, a, b):
        ...
        self.ensure(b)
        if ANCHORS <= 0:                      # anchor mode needs the past kept
            for k in list(self.buf):
                if k < a:
                    del self.buf[k]
        return [self.buf[k] for k in range(a, min(b, self.kf_ptr)) if k in self.buf]

    def frames_at(self, idxs):
        """Cached frames for arbitrary keyframe-list indices (anchor mode)."""
        return [self.buf[k] for k in idxs if k in self.buf]
...
# --------------------------------------------------------------- pipeline --
def cond_partial_pass(api, frames, ext_global, known, K0, process_res, replace_known=False):
    """DA3 forward with camera tokens for the KNOWN views only.

    DA3's pose conditioning (model/cam_enc.py) encodes a pose per view and writes
    it into backbone slot 0 of that view; without conditioning the same slot
    holds a learned "src" token. Nothing in the architecture ties the two
    choices across views, so we give the views the chain already knows (the
    overlap) their true global pose and leave the new views on the learned
    token. The window's coordinate frame is then anchored to len(known) real
    cameras instead of being free, which is exactly the degree of freedom the
    per-window scale error lives in.

    known: indices into `frames` whose ext_global rows are valid; view 0 must be
    known (DA3 normalises the conditioning frame to view 0 = identity).
    Returns global w2c (N,4,4) for all views: known rows copied through, new rows
    from the conditioned prediction mapped by a Sim(3) fitted on the known views.
    """
    from depth_anything_3.utils.geometry import affine_inverse
    net = api.model.da3                              # any-view net that owns cam_enc
    N = len(frames)
    known = list(known)
    if 0 not in known:
        raise RuntimeError("view 0 must be a known view")
    ext_full = np.tile(np.eye(4, dtype=np.float32)[None], (N, 1, 1))
    ext_full[known] = ext_global[known].astype(np.float32)
    ixt_full = np.tile(K0[None].astype(np.float32), (N, 1, 1))

    imgs_cpu, ext_pp, ixt_pp = api._preprocess_inputs(
        frames, ext_full, ixt_full, process_res, "upper_bound_resize")
    imgs, ex_t, in_t = api._prepare_model_inputs(imgs_cpu, ext_pp, ixt_pp)   # (1,N,...)
    dev = imgs.device
    kidx = torch.as_tensor(known, device=dev)

    # normalise the KNOWN subset the way api._normalize_extrinsics does for all
    ex_k = ex_t[:, kidx].float()
    ex_k = ex_k @ affine_inverse(ex_k[:, :1])
    c2w = affine_inverse(ex_k)
    med = torch.clamp(torch.median(c2w[..., :3, 3].norm(dim=-1)), min=1e-1)
    ex_k[..., :3, 3] = ex_k[..., :3, 3] / med

    H, W = imgs.shape[-2:]
    with torch.no_grad(), torch.autocast(device_type="cuda", enabled=False):
        tok_k = net.cam_enc(ex_k, in_t[:, kidx].float(), (H, W))           # (1,k,D)
    # DinoV2 is a wrapper; the learned tokens live on the inner ViT (.pretrained)
    vit = getattr(net.backbone, "pretrained", net.backbone)
    src = vit.camera_token[:, 1:]                                           # (1,1,D)
    cam_token = src.expand(1, N, -1).clone().to(tok_k.dtype)
    cam_token[:, kidx] = tok_k

    class _ConstToken(torch.nn.Module):          # nn.Module refuses a bare lambda
        def __init__(self, tok):
            super().__init__()
            self.tok = tok
        def forward(self, e, i, hw):
            return self.tok
    orig = net.cam_enc
    net.cam_enc = _ConstToken(cam_token)
    try:
        with torch.no_grad():
            raw = api._run_model_forward(imgs, ex_t, in_t, [], False)
    finally:
        net.cam_enc = orig
    pred = api._convert_to_prediction(raw)
    ext_pred = as_4x4(pred.extrinsics)

    # Sim(3) from the conditioned prediction frame to the global frame, fitted on
    # the known views (rotation from orientations, as align_window does).
    C_p, R_p = centers_rots(ext_pred)
    C_g, R_g = centers_rots(ext_global.astype(np.float64))
    kp, kg = C_p[known], C_g[known]
    M = sum(R_g[i] @ R_p[i].T for i in known)
    RA = proj_so3(M)
    dp, dg = kp - kp.mean(0), kg - kg.mean(0)
    den = (dp ** 2).sum()
    sA = float((dg * (RA @ dp.T).T).sum() / den) if den > 1e-12 else 1.0
    if not (np.isfinite(sA) and 0.05 < sA < 20.0):
        sA = 1.0
    tA = kg.mean(0) - sA * RA @ kp.mean(0)

    out = ext_global.astype(np.float64).copy()
    for j in range(N):
        if j in known and not replace_known:
            continue
        C = sA * RA @ C_p[j] + tA
        R_wc = RA @ R_p[j]
        E = np.eye(4)
...
def process_sequence(model, video_path, log):
    ...
    vid = VideoWindow(video_path, kf, rect_maps, crop_box)

    step = max(CHUNK - OVERLAP, 1)
    starts = list(range(0, max(n_kf - OVERLAP, 1), step))
    if PHASE > 0:
        starts = [0] + [t for t in range(PHASE, max(n_kf - OVERLAP, 1), step) if t > 0]
    ...
    for ci, s0 in enumerate(starts):
        ...
        anchor_idx = []
        if ANCHORS > 0 and ci > 0:
            off = step
            while len(anchor_idx) < ANCHORS:
                off *= ANCHOR_BASE
                k = s0 - off
                if k < 0:
                    break
                if glob_w2c[k] is not None and k in vid.buf:
                    anchor_idx.append(k)
            anchor_idx = sorted(set(anchor_idx))
            frames = vid.frames_at(anchor_idx) + frames
        sel = anchor_idx + list(range(s0, s1))
        ...
        shared = [k for k in sel if glob_w2c[k] is not None]
        if not shared:
        ...
        if COND in ("twopass", "partial") and shared:
            shared_set = set(shared)
            new = [k for k in range(s0, s1) if k not in shared_set]
            ext_in = np.stack([glob_w2c[k] for k in range(s0, s1)]).astype(np.float32)
            if rect_K is not None:
                K0 = rect_K
            else:   # no rectification (sim): un-scale DA3's own K to the input size
                Hp, Wp = depth.shape[1:]
                H0, W0 = frames[0].shape[:2]
                K0 = intr.mean(0).copy()
                K0[0, :] *= W0 / Wp
                K0[1, :] *= H0 / Hp
            ixt_in = np.tile(K0[None].astype(np.float32), (len(frames), 1, 1))
            try:
                if COND == "twopass":
                    ext2 = cond_partial_pass(model, frames, ext_in.astype(np.float64),
                                             list(range(len(frames))), K0, PROCESS_RES,
                                             replace_known=True)
                else:
                    ext2 = cond_partial_pass(model, frames, ext_in.astype(np.float64),
                                             [k - s0 for k in shared], K0, PROCESS_RES)
                C_in, _ = centers_rots(ext_in.astype(np.float64))
                C_2, _ = centers_rots(ext2)
                span = np.linalg.norm(C_in - C_in.mean(0), axis=1).mean() + 1e-9
                mv = np.linalg.norm(C_2 - C_in, axis=1) / span
                mv_new = float(np.mean([mv[k - s0] for k in new])) if new else 0.0
                mv_sh = float(np.mean([mv[k - s0] for k in shared]))
                cond_log.append((mv_new, mv_sh))
                for k in new:
                    glob_w2c[k] = ext2[k - s0]
            except Exception as exc:
                log(f"    chunk {ci+1}: cond pass failed ({type(exc).__name__}: {exc})")
        ...
        for j in range(len(anchor_idx) + (0 if ci == 0 else OVERLAP), len(sel), POINT_EVERY):
            d = depth[j][gy, gx]
    ...
    if cond_log:
        a = np.array(cond_log)
        log(f"  cond pass: new-view centre shift {a[:,0].mean():.3f} / shared {a[:,1].mean():.3f} "
            f"(x window extent; ~0 = model echoed the input, >0 = refined)")
    vid.release()
...
def main():
    ...
    # expB09: optional LoRA adapter (self-supervised colonoscopy adaptation)
    lora_path = os.environ.get("DA3_LORA", "")
    if lora_path:
        sys.path.insert(0, "/data4/src/shunsuke/MICCAI2026/CLiMB/workspace/expB09_lora/scripts")
        ...
        log(f"LoRA loaded from {lora_path}: {len(sd)} tensors, unexpected={len(unexpected)}")
