...
#
#   docker tag climb-da3-stride:v003 docker.synapse.org/syn<YOUR_PROJECT_ID>/climb-da3:v003
#   docker login docker.synapse.org        # password = Synapse Personal Access Token
#   docker push docker.synapse.org/syn<YOUR_PROJECT_ID>/climb-da3:v003
#
...
if [[ -n "${SYNAPSE_PROJECT_ID:-}" ]]; then
  REMOTE="docker.synapse.org/${SYNAPSE_PROJECT_ID}/climb-da3:v003"
  echo "== tag -> ${REMOTE} =="
