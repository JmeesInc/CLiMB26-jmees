...
#
#   docker tag climb-da3-rectify:v002 docker.synapse.org/syn<YOUR_PROJECT_ID>/climb-da3:v002
#   docker login docker.synapse.org        # password = Synapse Personal Access Token
#   docker push docker.synapse.org/syn<YOUR_PROJECT_ID>/climb-da3:v002
#
...
IMAGE="${IMAGE:-climb-da3-rectify:v002}"
...
if [[ -n "${SYNAPSE_PROJECT_ID:-}" ]]; then
  REMOTE="docker.synapse.org/${SYNAPSE_PROJECT_ID}/climb-da3:v002"
  echo "== tag -> ${REMOTE} =="
