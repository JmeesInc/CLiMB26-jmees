...
class VideoWindow:
    ...
    def __init__(self, path, rect_maps=None, crop_box=None):
        self.cap = cv2.VideoCapture(str(path))
        if not self.cap.isOpened():
            raise RuntimeError(f"cannot open video: {path}")
        self.fps = float(self.cap.get(cv2.CAP_PROP_FPS)) or 30.0
        self.rect_maps = rect_maps          # (mapx, mapy) kb4 -> pinhole, or None
        self.crop_box = crop_box            # (x0, x1, y0, y1) valid-content crop
        self.buf = {}
        self.next_read = 0
...
def process_sequence(model, video_path, log):
    ...
    vid = VideoWindow(video_path, rect_maps, crop_box)
