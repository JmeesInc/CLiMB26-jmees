...
#
#   docker tag climb-da3-dense:v005 docker.synapse.org/syn<YOUR_PROJECT_ID>/climb-da3:v005
#   docker login docker.synapse.org        # password = Synapse Personal Access Token
#   docker push docker.synapse.org/syn<YOUR_PROJECT_ID>/climb-da3:v005
#
...
if [[ -n "${SYNAPSE_PROJECT_ID:-}" ]]; then
  REMOTE="docker.synapse.org/${SYNAPSE_PROJECT_ID}/climb-da3:v005"
  echo "== tag -> ${REMOTE} =="
