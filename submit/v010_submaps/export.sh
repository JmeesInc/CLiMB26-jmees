...
#
#   docker tag climb-submaps:v010 docker.synapse.org/syn<YOUR_PROJECT_ID>/climb-da3:v010
#   docker login docker.synapse.org        # password = Synapse Personal Access Token
#   docker push docker.synapse.org/syn<YOUR_PROJECT_ID>/climb-da3:v010
#
...
if [[ -n "${SYNAPSE_PROJECT_ID:-}" ]]; then
  REMOTE="docker.synapse.org/${SYNAPSE_PROJECT_ID}/climb-da3:v010"
  echo "== tag -> ${REMOTE} =="
