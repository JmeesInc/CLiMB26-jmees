...
#
#   docker tag climb-pi3x:v009 docker.synapse.org/syn<YOUR_PROJECT_ID>/climb-da3:v009
#   docker login docker.synapse.org        # password = Synapse Personal Access Token
#   docker push docker.synapse.org/syn<YOUR_PROJECT_ID>/climb-da3:v009
#
...
if [[ -n "${SYNAPSE_PROJECT_ID:-}" ]]; then
  REMOTE="docker.synapse.org/${SYNAPSE_PROJECT_ID}/climb-da3:v009"
  echo "== tag -> ${REMOTE} =="
