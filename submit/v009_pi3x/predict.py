...
# Frames per emitted sub-map (0 = single map). See v010_submaps.
...
def write_runs(out_root, seq, frame_ids, extr, pts, cols, fps, init_s, proc_s):
    ...
    MIN_SPAN = 240
    if MAP_FRAMES > 0:
        n_maps = max(1, int(round(n / MAP_FRAMES)))
        while n_maps > 1 and n / n_maps < MIN_SPAN:
            n_maps -= 1
    else:
        n_maps = 1
    ...
    for run in range(1, NUM_RUNS + 1):
        ...
        for m in range(n_maps):
            ...
            write_trajectory(rd / "camera_trajectory" / f"cam_traj_map_{m:03d}.txt",
            ...
            npts += write_points3d(rd / "3D_maps" / f"{m:03d}" / "points3D.txt",
...
def process_sequence(model, video_path, log):
    ...
    for ci, s0 in enumerate(starts):
        ...
        # Per-keyframe depth for the pose-only BA in rotavg.finalize: cache the
        # depth from the window that first solves the frame, so it matches the
        # pose that gets refined. Kept at model resolution (fp16, ~40 MB/clip).
        if ROTAVG and int(os.environ.get("DA3_ROT_BA", "0")):
            for k in range(s0, s1):
                fa = kf[k]
                if fa not in depth_cache and glob_w2c[k] is None:
                    depth_cache[fa] = (depth[k - s0].astype(np.float16), intr[k - s0].copy())
    ...
    if rot_worker is not None:
        try:
            import rotavg as _rotavg
            extr_g = _rotavg.finalize(rot_worker, extr_g, log=log, depth_cache=depth_cache)
        except Exception as exc:
...
def main():
    ...
    if lora_path and os.path.isfile(lora_path):
        ...
        # Both backbones are DINOv2 ViTs with the same blocks[i].attn.{qkv,proj}
        # layout, so the injector is shared; only the module path differs.
        vit = (model.model.encoder if BACKEND == "pi3"
               else model.model.da3.backbone.pretrained)
        inject_lora(vit, r=int(os.environ.get("DA3_LORA_R", 8)),
                    alpha=int(os.environ.get("DA3_LORA_ALPHA", 16)))
