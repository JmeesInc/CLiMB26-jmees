...
#
#   docker tag climb-da3-rotfast:v007 docker.synapse.org/syn<YOUR_PROJECT_ID>/climb-da3:v007
#   docker login docker.synapse.org        # password = Synapse Personal Access Token
#   docker push docker.synapse.org/syn<YOUR_PROJECT_ID>/climb-da3:v007
#
...
IMAGE="${IMAGE:-climb-da3-rotfast:v007}"
...
if [[ -n "${SYNAPSE_PROJECT_ID:-}" ]]; then
  REMOTE="docker.synapse.org/${SYNAPSE_PROJECT_ID}/climb-da3:v007"
  echo "== tag -> ${REMOTE} =="
