...
class VideoWindow:
    ...
    def __init__(self, path, kf_indices, rect_maps=None, crop_box=None):
        ...
        if rect_maps is not None:
            self.rot_maps = (
                cv2.resize(rect_maps[0], None, fx=ROT_SCALE, fy=ROT_SCALE,
                           interpolation=cv2.INTER_LINEAR),
                cv2.resize(rect_maps[1], None, fx=ROT_SCALE, fy=ROT_SCALE,
                           interpolation=cv2.INTER_LINEAR))
...
def process_sequence(model, video_path, log):
    ...
    if ROTAVG and rect_maps is not None:
        try:
            ...
            K_small = rect_maps[2].copy(); K_small[:2] *= ROT_SCALE
            if crop_box is not None:
                K_small[0, 2] -= int(crop_box[0] * ROT_SCALE)
                K_small[1, 2] -= int(crop_box[2] * ROT_SCALE)
