...
ROT_CODEC = os.environ.get("DA3_ROT_CODEC", "png")
...
class VideoWindow:
    ...
    def _rot_store(self, idx, frame_bgr, already_rect):
        """Half-res rectified BGR frame, jpg-encoded (memory-safe on long clips)."""
        f = frame_bgr
        if not already_rect:
            if self.rect_maps is not None:
                f = cv2.remap(f, self.rect_maps[0], self.rect_maps[1],
                              cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT,
                              borderValue=(0, 0, 0))
            if self.crop_box is not None:
                x0, x1, y0, y1 = self.crop_box
                f = f[y0:y1, x0:x1]
        half = cv2.resize(f, None, fx=ROT_SCALE, fy=ROT_SCALE, interpolation=cv2.INTER_AREA)
        if ROT_CODEC == "png":   # lossless: quarter-res nodes are ~150KB, OK in RAM
            ok, enc = cv2.imencode(".png", half, [cv2.IMWRITE_PNG_COMPRESSION, 1])
        else:
            ok, enc = cv2.imencode(".jpg", half, [cv2.IMWRITE_JPEG_QUALITY, 92])
        if ok:
...
def process_sequence(model, video_path, log):
    ...
    if ROTAVG and vid.rot_cache and rect_K is not None:
        try:
            import rotavg as _rotavg
            K_half = rect_K.copy(); K_half[:2] *= ROT_SCALE
            extr_g = _rotavg.refine(extr_g, vid.rot_cache, K_half, log=log)
