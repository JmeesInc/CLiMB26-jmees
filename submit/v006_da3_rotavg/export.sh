...
#
#   docker tag climb-da3-rotavg:v006 docker.synapse.org/syn<YOUR_PROJECT_ID>/climb-da3:v006
#   docker login docker.synapse.org        # password = Synapse Personal Access Token
#   docker push docker.synapse.org/syn<YOUR_PROJECT_ID>/climb-da3:v006
#
...
IMAGE="${IMAGE:-climb-da3-rotavg:v006}"
...
if [[ -n "${SYNAPSE_PROJECT_ID:-}" ]]; then
  REMOTE="docker.synapse.org/${SYNAPSE_PROJECT_ID}/climb-da3:v006"
  echo "== tag -> ${REMOTE} =="
