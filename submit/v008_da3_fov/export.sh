...
#
#   docker tag climb-da3-fov:v008 docker.synapse.org/syn<YOUR_PROJECT_ID>/climb-da3:v008
#   docker login docker.synapse.org        # password = Synapse Personal Access Token
#   docker push docker.synapse.org/syn<YOUR_PROJECT_ID>/climb-da3:v008
#
...
IMAGE="${IMAGE:-climb-da3-fov:v008}"
...
if [[ -n "${SYNAPSE_PROJECT_ID:-}" ]]; then
  REMOTE="docker.synapse.org/${SYNAPSE_PROJECT_ID}/climb-da3:v008"
  echo "== tag -> ${REMOTE} =="
